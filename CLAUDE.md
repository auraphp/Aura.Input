...
- [src/Filter.php](src/Filter.php) is the bundled `FilterInterface` implementation: closure-based rules keyed by field, applied in `apply(array|object $values): FilterResultInterface` where `$values` is the Fieldset itself (closures receive `$value` and `$fields`, where `$fields` is the Fieldset and `$value` is `$fields->$field` via `__get` → `Field::read()`). Because the Fieldset *is* the subject, closures write back through the object handle and the Fieldset passed in is modified in place. Failures use the bundled [src/Filter/FailureCollection.php](src/Filter/FailureCollection.php), which stores [src/Filter/Failure.php](src/Filter/Failure.php) value objects per field — keyed by field name or dot-notation path — so the `$args` given to `add()`/`set()` are retained; `addMessagesForField()` is the bulk helper used by `Filter::apply()` and Fieldset aggregation.
